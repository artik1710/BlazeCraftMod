...
public class TileentityBlazeFurnance extends TileEntity implements ITickable, IInventory {
	...
	public ITextComponent getDisplayName() {
		return this.hasCustomName() ? new TextComponentString(this.customName)
				: new TextComponentTranslation("container.blaze_furnace");
	}
	...
	public static boolean isBurning(TileentityBlazeFurnance te) {
	...
	public void update() {
		if (this.isBurning()) {
			--this.burnTime;
			BlockBlazeFurnace.setState(true, world, pos);
		}
		...
		if (this.isBurning() && this.canSmelt() && cookTime > 0) {
			...
			if (this.canSmelt() && this.isBurning()) {
				ItemStack output = BlazeFurnaceRecipes.getInstance().getSinteringResult(inputs[0], inputs[1]);
				if (!output.isEmpty()) {
					smelting = output;
					cookTime++;
					inputs[0].shrink(1);
	...
	private boolean canSmelt() {
		if (((ItemStack) this.handler.getStackInSlot(0)).isEmpty()
				|| ((ItemStack) this.handler.getStackInSlot(1)).isEmpty())
			return false;
		else {
			ItemStack result = BlazeFurnaceRecipes.getInstance().getSinteringResult(
					(ItemStack) this.handler.getStackInSlot(0), (ItemStack) this.handler.getStackInSlot(1));
			if (result.isEmpty())
				return false;
			else {
	...
	public String getName() {
		return this.hasCustomName() ? this.customName: "container.blaze_furnace";
	}
